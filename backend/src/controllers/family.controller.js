...
export async function createFamily(req, res) {
  ...
  const checkResult = await client`SELECT EXISTS (SELECT 1 FROM families WHERE family_name = ${family_name} AND created_user = ${userid})`;

  try {
    if (!family_name) {
      return res.status(400).json({ message: "Family Name required" });
    } else if (checkResult[0].exists) {
      return res.status(400).json({ message: "Family Name Already Exist" });
    } else {
      await client`INSERT INTO families (family_name , created_user) VALUES (${family_name}, ${userid})`;
...
export async function getFamily(req, res) {
  ...
  try {
    if (!family_id) {
      ...
      const result = await client`SELECT family_id,family_name FROM families WHERE family_id = ${family_id} AND created_user = ${userid}`;
...
export async function getAllFamily(req, res) {
  const userid = req.user.userid;
  try {
    const result = await client`SELECT family_id, family_name FROM families WHERE created_user = ${userid}`;
...
export async function editFamily(req, res) {
  ...
  const res_old = await client`SELECT family_name FROM families WHERE family_id = ${family_id}`;
  const oldFamilyName = res_old[0]?.family_name;
  ...
  try {
    ...
    const result = await client`UPDATE families SET family_name = ${family_name} WHERE family_id = ${family_id} RETURNING *`;
...
export async function deleteAllFamily(req, res) {
  const userid = req.user.userid;
  try {
    await client`DELETE FROM families WHERE created_user = ${userid}`;
